...
func (c *Connack) Decode(m Mode, src []byte) (int, error) {
	...
	if err != nil {
		return total, wrapError(CONNACK, m, err)
	}
	...
	if connackFlags&254 != 0 {
		return total, makeError(CONNACK, m, "bits 7-1 in acknowledge flags are not 0")
	}
	...
	if !returnCode.Valid() {
		return total, makeError(CONNACK, m, "invalid return code (%d)", c.ReturnCode)
	}
	...
	// check buffer
	if len(src[total:]) != 0 {
		return total, makeError(CONNACK, m, "leftover buffer length (%d)", len(src[total:]))
	}
