...
func (s *Suback) Decode(m Mode, src []byte) (int, error) {
	...
	if err != nil {
		return total, wrapError(SUBACK, DECODE, m, total, err)
	}
	...
	s.ID = ID(pid)
	if !s.ID.Valid() {
		return total, wrapError(SUBACK, DECODE, m, total, ErrInvalidPacketID)
	}

	// calculate number of return codes
	rcl := rl - 2
	if rcl < 1 {
		return total, makeError(SUBACK, DECODE, m, total, "missing return codes")
	}
	...
	for i := 0; i < rcl; i++ {
		...
		if !returnCode.Successful() && returnCode != QOSFailure {
			return total, makeError(SUBACK, DECODE, m, total, "invalid return code")
		}
...
func (s *Suback) Encode(m Mode, dst []byte) (int, error) {
	...
	if err != nil {
		return total, wrapError(SUBACK, ENCODE, m, total, err)
	}

	// check packet id
	if !s.ID.Valid() {
		return total, wrapError(SUBACK, ENCODE, m, total, ErrInvalidPacketID)
	}
	...
	for _, rc := range s.ReturnCodes {
		...
		if !rc.Successful() && rc != QOSFailure {
			return total, makeError(SUBACK, ENCODE, m, total, "invalid return code")
		}
